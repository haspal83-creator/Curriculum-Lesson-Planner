...
export function validateAndBalanceLessonTiming(
  ...
  const targetMinutes = parseMinutes(selectedDuration || plan.duration, 45);
  ...
  // If no lessonAtAGlance rows exist yet, seed standard 5 stages from executionBoard or defaults
  if (rows.length === 0) {
  ...
  if (needsRebalance) {
    // Determine target minutes for each row proportionally
    const count = rows.length;
    let distributed: number[] = [];

    if (count === 5) {
      // Use refined primary pedagogy ratios
    ...
    let adjustIdx = count >= 2 ? 1 : 0; // Bias adjustment towards explicit teaching / practice
    while (diff !== 0) {
      if (diff > 0) {
